...
function parseInput(inputText) {
    ...
    inputText.split('\n').forEach((el, i) => {
        ...
        parsedInput.push({
            'pass': parts[1], 
            'rules': [{
                'letter': leftSidePart[1],
                'position': [parseInt(minMax[0]) - 1, parseInt(minMax[1]) - 1], // [must have the letter, mustn't have]
            }]
        });
    });
...
function resolve(input) {
    ...
    input.forEach((el, i) => {
        el.rules.forEach((rule, j) => {
            const firstPosition = el.pass[rule.position[0]] === rule.letter;
            const secondPosition = el.pass[rule.position[1]] === rule.letter;
