const inputText = `1-4 m: mrfmmbjxr
...
function parseInput(inputText) {
    ...
    inputText.split('\n').forEach((el, i) => {
        ...
        const leftSidePart = parts[0].split(' ');
        const minMax = leftSidePart[0].split('-');

        parsedInput.push({
            'pass': parts[1], // right side of the input
            'rules': [{
                'letter': leftSidePart[1], // right side of the pattern
                'min': parseInt(minMax[0]), // min
                'max': parseInt(minMax[1])   // max
            }]
        });
    });
...
function resolve(input) {
    ...
    input.forEach((el, i) => {
        ...
        const map = passwordToMap(el.pass); // map with letter and repeated number of letters

        el.rules.forEach((el, j) => {
            if ((map[el['letter']] >= el['min']) && (map[el['letter']] <= el['max'])) {
                ++count;
            }
        });
    });
